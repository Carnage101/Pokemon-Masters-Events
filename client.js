...
document.getElementById("rumors").onclick = () => {
  ...
  setInterval(() => {
    ...
    document.getElementById("rumorstitles").innerHTML = nextEvents.length >= 1 ? "<b style=\"\">" + nextEvents.map(e => e.name.map(a => a.replace(/\[grid\]/g, "<img src=\"./grid.png\"/>").replace(/\[ex\]/g, "<img src=\"./ex.png\"/>").replace(/\[gridEX\]/g, "<img src=\"./grid.png\"/><img src=\"./ex.png\"/>"))).flat().join("<br>") + "</b>" : "No rumors yet.";
    ...
    document.getElementById("futurerumors").innerHTML = nextEvents.length >= 1 ? futureEvents.map(e => e.name.map(n => `${n} - <span style="color:#ffc361;">${new Date(e.date + " 06:00:00 UTC").toDateString()}</span>`.replace(/\[grid\]/g, "<img src=\"./grid.png\"/>").replace(/\[ex\]/g, "<img src=\"./ex.png\"/>").replace(/\[gridEX\]/g, "<img src=\"./grid.png\"/><img src=\"./ex.png\"/>"))).flat().join("<br>") : null;
...
document.getElementById("ongoing").onclick = () => {
  ...
  setInterval(() => {
    ...
    document.getElementById("ongoinglist").innerHTML = events.sort((a, b) => new Date(a.expires + " 06:00:00 UTC").getTime() - new Date(b.expires + " 06:00:00 UTC").getTime()).map(e => e.name.map(n => `${n} - Ends on <span style="color:#ffc361;">${new Date(e.expires + " 06:00:00 UTC").toDateString()}</span>`.replace("[grid]", "<img src=\"./grid.png\"/>").replace("[ex]", "<img src=\"./ex.png\"/>").replace("[gridEX]", "<img src=\"./grid.png\"/><img src=\"./ex.png\"/>"))).flat().join("<br>") || "No events running at the moment.";
